...
try:
    ...
    OCR_AVAILABLE = True
    if OCR_AVAILABLE:
        custom_tess_cmd = os.environ.get('TESSERACT_CMD') or 'F:\\\\Tesseract\\\\tesseract.exe'
...
class DocumentParser:
    ...
    def __init__(self):
        self.supported_extensions = {'.pdf': self._parse_pdf, '.docx': self._parse_docx, '.doc': self._parse_doc, '.xlsx': self._parse_xlsx, '.xls': self._parse_xls, '.net': self._parse_net_file, '.cs': self._parse_net_file, '.vb': self._parse_net_file, '.aspx': self._parse_net_file, '.ashx': self._parse_net_file, '.asmx': self._parse_net_file, '.config': self._parse_net_file, '.csproj': self._parse_net_file, '.vbproj': self._parse_net_file, '.sln': self._parse_net_file, '.dll': self._parse_binary_file, '.exe': self._parse_binary_file, '.pdb': self._parse_binary_file, '.pptx': self._parse_pptx, '.png': self._parse_image, '.jpg': self._parse_image, '.jpeg': self._parse_image, '.zip': self._parse_zip, '.txt': self._parse_txt, '.md': self._parse_txt, '.csv': self._parse_txt}
    ...
    def _get_extension_from_mime(self, mime_type: str) -> Optional[str]:
        mime_to_ext = {'application/pdf': '.pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document': '.docx', 'application/msword': '.doc', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': '.xlsx', 'application/vnd.ms-excel': '.xls', 'text/plain': '.txt', 'text/x-csharp': '.cs', 'text/x-vb': '.vb', 'application/xml': '.xml'}
    ...
    def _parse_pdf(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        if image_jobs:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = [executor.submit(self._ocr_pdf_image, page_num, img_index, img_data) for page_num, img_index, img_data in image_jobs]
        ...
        return {'text': text_content, 'tables': tables_data, 'file_type': 'pdf'}
    ...
    def _parse_docx(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'text': text_content, 'tables': tables_data, 'file_type': 'docx'}
    ...
    def _parse_doc(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'text': text_content, 'tables': [], 'file_type': 'doc'}
    ...
    def _parse_xlsx(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'text': text_content, 'tables': tables_data, 'file_type': 'xlsx'}
    ...
    def _parse_xls(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'text': text_content, 'tables': tables_data, 'file_type': 'xls'}
    ...
    def _parse_net_file(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'text': text_content, 'tables': [], 'file_type': 'net'}
    ...
    def _parse_binary_file(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        return {'tables': [], 'file_type': 'binary'}
    ...
    def _parse_image(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        try:
            ...
            return {'text': f'Error reading image file: {e}', 'tables': [], 'file_type': 'image'}
        tesseract_text = ''
        if OCR_AVAILABLE:
            try:
        ...
        return {'text': text, 'tables': [], 'file_type': 'image'}
    ...
    def _parse_pptx(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        ...
        def _process_slide(slide_index: int, slide) -> Tuple[str, list[Dict]]:
            ...
            for shape in slide.shapes:
                ...
                try:
                    if getattr(shape, 'shape_type', None) == 13 and hasattr(shape, 'image'):
                        ...
                        local_images.append({'slide': slide_index + 1, 'image_format': image.ext, 'image_base64': image_b64})
                        slide_lines.append(f'[Image on Slide {slide_index + 1}: {image.ext}, {len(image_bytes)} bytes]')
            ...
            if slide_image_bytes:
                if len(slide_image_bytes) == 1:
                    ...
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        futures = [executor.submit(_ocr_slide_image, slide_index, image_bytes) for image_bytes in slide_image_bytes]
                        for future in as_completed(futures):
                            try:
        ...
        return {'text': text_content, 'images': images_data, 'tables': [], 'file_type': 'pptx'}
    ...
    def _extract_table_from_docx(self, table_element) -> Dict:
        ...
        return {'data': table_data, 'text': table_text, 'rows': len(table_data), 'columns': len(table_data[0]) if table_data else 0}
    ...
    def _extract_table_from_xlsx(self, sheet, sheet_name: str) -> Dict:
        ...
        return {'data': table_data, 'text': table_text, 'rows': len(table_data), 'columns': len(table_data[0]) if table_data else 0, 'sheet_name': sheet_name}
    ...
    def extract_tables_from_content(self, content: Dict) -> List[Dict]:
        ...
        for i, table in enumerate(tables):
            formatted_table = {'table_index': i, 'rows': table.get('rows', 0), 'columns': table.get('columns', 0), 'data': table.get('data', []), 'text': table.get('text', ''), 'sheet_name': table.get('sheet_name', '')}
            formatted_tables.append(formatted_table)
        return formatted_tables

    def _parse_txt(self, file_path: Path) -> Dict[str, Union[str, List[Dict]]]:
        try:
            text = file_path.read_text(encoding='utf-8')
        except UnicodeDecodeError:
            text = file_path.read_text(encoding='latin-1')
        return {'text': text, 'tables': [], 'file_type': file_path.suffix.lstrip('.')}
